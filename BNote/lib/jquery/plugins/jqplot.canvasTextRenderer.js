...
(function ($) {
  ...
  $.jqplot.CanvasTextRenderer.prototype.measure = function (ctx, str) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (c) {
        total +=
          ((c.width * this.normalizedFontSize) / 25.0) * this.fontStretch;
      }
    }
    return total;
  };
  ...
  $.jqplot.CanvasTextRenderer.prototype.draw = function (ctx, str) {
    ...
    // 2nd quadrant
      ...
      ty =
        -Math.sin(this.angle) * this.width - Math.cos(this.angle) * this.height;
    }
    // 3rd quadrant
    else if (
      ...
      tx =
        Math.sin(this.angle) * this.height - Math.cos(this.angle) * this.width;
