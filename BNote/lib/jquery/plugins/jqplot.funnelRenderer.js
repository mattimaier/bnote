...
(function ($) {
    ...
    if (this.highlightColors.length == 0) {
      for (var i = 0; i < this.seriesColors.length; i++) {
        ...
        for (var j = 0; j < 3; j++) {
          ...
          newrgb[j] =
            sum > 570 ? newrgb[j] * 0.8 : newrgb[j] + 0.4 * (255 - newrgb[j]);
  ...
  $.jqplot.FunnelRenderer.prototype.drawSection = function (
    ctx,
    vertices,
    color,
    isShadow
  ...
  $.jqplot.FunnelRenderer.prototype.draw = function (ctx, gd, options, plot) {
    ...
    this._atot =
      (ltot / 2) * (this._bases[0] + this._bases[0] * this.widthRatio);
    ...
    for (i = 0; i < this._areas.length; i++) {
      ...
      while (err > this._lengths[i] * tolerance && count < 100) {
        this._lengths[i] =
          this._areas[i] /
          (this._bases[i] - this._lengths[i] * Math.tan(this._angle));
        err = Math.abs(this._lengths[i] - guess);
        this._bases[i + 1] =
          this._bases[i] - 2 * this._lengths[i] * Math.tan(this._angle);
    ...
    var p0 = [loff, toff],
      ...
      p2 = [
        loff + (this._bases[0] - this._bases[this._bases.length - 1]) / 2,
        toff + this._length,
      ],
    ...
    if (this.shadow) {
      ...
      for (var i = 0; i < gd.length; i++) {
        this.renderer.drawSection.call(
          this,
          ctx,
          this._vertices[i],
          shadowColor,
  ...
  $.jqplot.FunnelAxisRenderer.prototype.constructor =
  ...
  $.jqplot.FunnelLegendRenderer.prototype.constructor =
  ...
  function handleMove(ev, gridpos, datapos, neighbor, plot) {
    if (neighbor) {
      ...
      if (
        ...
        !(
          ins[0] == plot.plugins.funnelRenderer.highlightedSeriesIndex &&
  ...
  $.jqplot.FunnelTickRenderer.prototype.constructor =
    $.jqplot.FunnelTickRenderer;
})(jQuery);
