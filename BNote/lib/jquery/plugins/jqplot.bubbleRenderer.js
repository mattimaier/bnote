...
(function ($) {
  ...
  $.jqplot.BubbleRenderer.prototype.setGridData = function (plot) {
    ...
    var r,
      val,
      maxr = (this.maxRadius = arrayMax(radii));
  ...
  $.jqplot.BubbleRenderer.prototype.draw = function (ctx, gd, options) {
    ...
    for (var i = 0; i < this.radii.length; i++) {
      ...
      this.canvas._elem.append(
        this.bubbleCanvases[idx].createElement(gd[0], gd[1], canvasRadius)
      );
  ...
  $.jqplot.DivCanvas.prototype.createElement = function (
    offsets,
    clss,
    plotDimensions
  ) {
  ...
  $.jqplot.BubbleCanvas.prototype.draw = function (r, color, gradients, angle) {
    ...
    if (gradients && !$.jqplot.use_excanvas) {
      ...
      var colorinner =
        "rgba(" +
        Math.round(comps[0] + 0.8 * (255 - comps[0])) +
        ", " +
        Math.round(comps[1] + 0.8 * (255 - comps[1])) +
        ", " +
        Math.round(comps[2] + 0.8 * (255 - comps[2])) +
        ", " +
        comps[3] +
        ")";
      var colorend =
        "rgba(" + comps[0] + ", " + comps[1] + ", " + comps[2] + ", 0)";
  ...
  $.jqplot.BubbleAxisRenderer.prototype.constructor =
    $.jqplot.BubbleAxisRenderer;
  ...
  function highlight(plot, sidx, pidx) {
    ...
    if (s.labels[pidx]) {
      ...
      plot.plugins.bubbleRenderer.highlightLabel.appendTo(
        plot.plugins.bubbleRenderer.highlightLabelCanvas
      );
      plot.plugins.bubbleRenderer.highlightLabel.addClass(
  ...
  function handleMove(ev, gridpos, datapos, neighbor, plot) {
    if (neighbor) {
      ...
      if (
        ...
        !(
          ins[0] == plot.plugins.bubbleRenderer.highlightedSeriesIndex &&
