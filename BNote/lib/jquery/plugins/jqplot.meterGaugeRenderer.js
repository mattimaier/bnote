...
(function ($) {
  ...
  $.jqplot.MeterGaugeRenderer.prototype.constructor =
  ...
  $.jqplot.MeterGaugeRenderer.prototype.draw = function (ctx, gd, options) {
    ...
    if (!this.diameter) {
      if (this.semiCircular) {
        ...
        this._center = [
          (cw - trans * offx) / 2 + trans * offx,
          ch + trans * offy - this.padding - this.ringWidth - this.innerPad,
        ];
      } else {
        if (!this.ringWidth) {
          this.ringWidth = d / 35;
    ...
    if (this.ticks.length == 0) {
      ...
      var tf = Math.floor(
        parseFloat((Math.log(ti) / Math.log(10)).toFixed(11))
      );
      ...
      if (setmax && setmin) {
        ...
        if (!this.forceZero) {
          ...
          if (ntp > 1) {
            ...
            if (
              parseInt(tmin, 10) != tmin &&
              parseInt(tmin - t[tpindex], 10) == tmin - t[tpindex]
            ) {
              tmin = tmin - t[tpindex];
            }
          }
      ...
      if (!setmax && !setmin) {
        ...
        tf =
          Math.floor(parseFloat((Math.log(range) / Math.log(10)).toFixed(11))) -
          1;
        var nticks = [5, 6, 4, 7, 3, 8, 9, 10, 2],
          res,
          numticks,
          nonSigDigits = 0,
          sigRange;
    ...
    if (this.semiCircular) {
      var overAngle = Math.atan(this.innerPad / r),
        ...
        hubsa = (this.hubStartAngle =
          sa - Math.atan((this.innerPad / this.hubRadius) * 2)),
        hubea = (this.hubEndAngle =
          ea + Math.atan((this.innerPad / this.hubRadius) * 2));
      ...
      if (this.showTicks) {
        ...
        for (i = 0; i < this.ticks.length; i++) {
          ...
          var wps = ts * i + sa;
          ctx.moveTo(
            -orad * Math.cos(ts * i + sa),
            orad * Math.sin(ts * i + sa)
          );
          ctx.lineTo(
            -(orad - tl) * Math.cos(ts * i + sa),
            (orad - tl) * Math.sin(ts * i + sa)
          );
          ...
          if (i < this.ticks.length - 1) {
            for (var j = 1; j <= nmt; j++) {
              ctx.beginPath();
              ctx.moveTo(
                -orad * Math.cos(ts * i + mts * j + sa),
                orad * Math.sin(ts * i + mts * j + sa)
              );
              ctx.lineTo(
                -(orad - mtl) * Math.cos(ts * i + mts * j + sa),
                (orad - mtl) * Math.sin(ts * i + mts * j + sa)
              );
              ctx.stroke();
            }
          }
      ...
      if (this.showTickLabels) {
        ...
        for (i = 0; i < this.ticks.length; i++) {
          ...
          l =
            this._tickPoints[i][0] -
            (ew * (this._tickPoints[i][2] - Math.PI)) / Math.PI -
            tp * Math.cos(this._tickPoints[i][2]);
          t =
            this._tickPoints[i][1] -
            eh / 2 +
            (eh / 2) *
              Math.pow(Math.abs(Math.sin(this._tickPoints[i][2])), 0.5) +
            (tp / 3) *
              Math.pow(Math.abs(Math.sin(this._tickPoints[i][2])), 0.5);
          ...
          dim =
            ew * Math.cos(this._tickPoints[i][2]) +
            eh * Math.sin(Math.PI / 2 + this._tickPoints[i][2] / 2);
      ...
      if (this.intervalOuterRadius == null) {
        if (this.showTickLabels) {
          ...
          var outer =
            this.tickOuterRadius - this.tickLength - this.diameter / 16;
        }
      } else {
      ...
      var start,
        ...
        span = (this.span * Math.PI) / 180;
      for (i = 0; i < this.intervals.length; i++) {
        start =
          i == 0
            ? sa
            : sa + ((this.intervals[i - 1][0] - this.min) * span) / range;
        if (start < 0) {
          start = 0;
        }
      ...
      var dataang =
        (((datapoint - this.min) / dataspan) * this.span * Math.PI) / 180 +
        this.startAngle;
      ...
      for (var i = 0; i < 10; i++) {
        ...
        ctx.lineWidth = templ;
        ctx.lineTo(
          dl * (i + 1) * Math.cos(dataang),
          dl * (i + 1) * Math.sin(dataang)
        );
        ctx.stroke();
      }
  ...
  $.jqplot.MeterGaugeAxisRenderer.prototype.constructor =
  ...
  $.jqplot.MeterGaugeLegendRenderer.prototype =
    new $.jqplot.TableLegendRenderer();
  $.jqplot.MeterGaugeLegendRenderer.prototype.constructor =
    $.jqplot.MeterGaugeLegendRenderer;
    ...
    if (this.show) {
      ...
      ss +=
        this.marginRight != null
          ...
          : "";
      this._elem = $(
        '<table class="jqplot-table-legend" style="' + ss + '"></table>'
      );
      ...
      if (s.show) {
        ...
        for (i = 0; i < nr; i++) {
          if (reverse) {
            tr = $('<tr class="jqplot-table-legend"></tr>').prependTo(
              this._elem
            );
          } else {
            tr = $('<tr class="jqplot-table-legend"></tr>').appendTo(
              this._elem
            );
          }
          for (j = 0; j < nc; j++) {
  ...
  function preInit(target, data, options) {
    ...
    if (setopts) {
      ...
      options.grid.borderWidth =
        options.grid.borderWidth != null ? options.grid.borderWidth : 0;
      options.grid.shadow =
        options.grid.shadow != null ? options.grid.shadow : false;
    }
  }
  ...
  $.jqplot.MeterGaugeTickRenderer.prototype.constructor =
    $.jqplot.MeterGaugeTickRenderer;
})(jQuery);
