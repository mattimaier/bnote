...
(function ($) {
  ...
  $.jqplot.CanvasOverlay.postPlotInit = function (target, data, opts) {
    ...
    this.plugins.canvasOverlay = new $.jqplot.CanvasOverlay(
  ...
  $.jqplot.CanvasOverlay.prototype.get =
    $.jqplot.CanvasOverlay.prototype.getObject;
    ...
    if (this.options.show) {
      ...
      for (var k = 0; k < objs.length; k++) {
        ...
        if (obj.options.show) {
          ...
          obj.tooltipWidthFactor =
            obj.options.lineWidth / obj.options.showTooltipPrecision;
          switch (obj.type) {
            case "line":
              // style and shadow properties should be set before
              // every draw of marker renderer.
              ...
              start = [
                plot.axes[obj.options.xaxis].series_u2p(obj.options.start[0]),
                plot.axes[obj.options.yaxis].series_u2p(obj.options.start[1]),
              ];
              stop = [
                plot.axes[obj.options.xaxis].series_u2p(obj.options.stop[0]),
                plot.axes[obj.options.yaxis].series_u2p(obj.options.stop[1]),
              ];
              obj.gridStart = start;
              obj.gridStop = stop;
            ...
            case "dashedVerticalLine":
              ...
              if (obj.options.x != null) {
                ...
                if (ystop != null && ystart != null) {
                  ...
                  var firstDashAdjust =
                    (numDash * dashPatLen - (ystart - ystop)) / 2.0;
                  var b = ystart,
                    e,
                    bs,
                    es;
  ...
  function showTooltip(plot, obj, gridpos, datapos) {
    ...
    elem.html(
      $.jqplot.sprintf(opts.tooltipFormatString, datapos[0], datapos[1])
    );
  ...
  function handleMove(ev, gridpos, datapos, neighbor, plot) {
    ...
    for (var i = 0; i < l; i++) {
      ...
      if (obj.options.showTooltip) {
        ...
        datapos = [
          plot.axes[obj.options.xaxis].series_p2u(gridpos.x),
          plot.axes[obj.options.yaxis].series_p2u(gridpos.y),
        ];
          ...
          switch (obj.type) {
            ...
            case "dashedHorizontalLine":
              showTooltip(
                plot,
                obj,
                [gridpos.x, obj.gridStart[1]],
                [datapos[0], obj.options.y]
              );
              break;
            ...
            case "dashedVerticalLine":
              showTooltip(
                plot,
                obj,
                [obj.gridStart[0], gridpos.y],
                [obj.options.x, datapos[1]]
              );
              break;
