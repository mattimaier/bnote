...
| `profile`                 | `/profile`, `/profile/edit`, `api/modules/profile.php`                                      | Supported           | Personal/contact-data management is available in Next Gen profile flows.                | Keep parity checks for legacy-only detail fields.                                                              |
...
- **Admin email (new registration):** After success, the API may notify administrators via the Next Gen mail stack (`api/mail/`, PHPMailer): one send with the first admin in **To** and the rest in **BCC**. Copy lives in `lang/*.json` (`mail.*`). Configure SMTP with `MAIL_*` env vars; deep links use `BNOTE_NEXT_GENERATION_PUBLIC_URL` (or `BNOTE_NEXT_GENERATION_PUBLIC_ORIGIN` + `NEXT_PUBLIC_BASE_PATH`). Failures are logged only. Details: **[MAIL.md](MAIL.md)**.
...
- **Event access precedence:** For rehearsals/concerts, **module rights override participant assignment**. Users with **`Proben`** or **`Konzerte`** module permission can still list/open those entities even when removed from `*_contact` participant relations. Participant relations remain relevant for per-user participation rows and invitation targeting.
