...
final class NextGenPersistentLogin {
    ...
    public static function issueAndSetCookie(int $userId, object $db): void {
        ...
        $db->execute(
            'INSERT INTO persistent_login_token (user_id, token_hash, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL ' . $ttl . ' SECOND))',
            [['i', $userId], ['s', $hash]]
        );
    ...
    public static function restoreSessionIfPossible(object $db): int {
        ...
        $db->execute(
            'UPDATE persistent_login_token SET revoked_at = NOW(), last_used_at = NOW() WHERE id = ? LIMIT 1',
            [['i', (int) $row['id']]]
        );
    ...
    public static function revokeCurrentTokenAndClearCookie(object $db): void {
        ...
        if ($plain !== '' && PersistentLoginTokenSchema::ensureTable($db)) {
            $hash = hash('sha256', $plain, false);
            $db->execute(
                'UPDATE persistent_login_token SET revoked_at = NOW() WHERE token_hash = ? AND revoked_at IS NULL',
                [['s', $hash]]
            );
        }
        self::clearCookie();
