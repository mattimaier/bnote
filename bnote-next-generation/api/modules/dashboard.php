...
class DashboardModule
{
  ...
  public function __construct()
  {
    ...
    global $system_data;
    $moduleId = getLegacyModuleId($system_data, LegacyModuleKey::DASHBOARD);
  ...
  private function getAllInboxItems()
  {
    ...
    $concertsModuleId = getLegacyModuleId($system_data, LegacyModuleKey::CONCERTS);
    ...
    for ($i = 1; $i < count($votesSel); $i++) {
      ...
      $participation = $isMultiDate
        ? ($this->hasUserVotedForAllOptions($vid, $userId)
          ? 1
          : -1)
        : ($this->hasUserVoted($vid, $userId)
          ? 1
          : -1);
    ...
    $calendarModuleId = getLegacyModuleId($system_data, LegacyModuleKey::CALENDAR);
  ...
  private function getUserRehearsalIds($userId)
  {
    ...
    $rehearsals = array_merge($this->getRehearsalsForUser($userId), $this->getRehearsalsForPhases($usersPhases));
  ...
  private function completeTask()
  {
    ...
    $canToggle =
      (int) ($system_data->dbcon->colValue(
  ...
  private function getActivityFeed()
  {
    ...
    if ($discussionOn) {
      ...
      if (count($allOids) > 0) {
        ...
        $params[] = ["i", 15];
        $query =
          "SELECT c.id, c.otype, c.oid, c.author, c.message, c.created_at,
  ...
  private function getAdminOverview()
  {
    ...
    $actionNeeded =
      count($participationGaps["events"]) +
  ...
  private function getAdminPendingAccounts($system_data)
  {
    ...
    $q =
      "SELECT u.id AS userId, c.id AS contactId, u.login, c.name, c.surname, c.email " .
  ...
  private function saveSimpleEscalationPairsToConfig(array $pairs): void
  {
    ...
    $existing = $system_data->dbcon->colValue("SELECT value FROM configuration WHERE param = ?", "value", [
      ["s", self::SIMPLE_ESCALATION_PAIR_PARAM],
    ]);
    if ($existing !== null && $existing !== false) {
      $system_data->dbcon->execute("UPDATE configuration SET value = ? WHERE param = ?", [
        ["s", $json],
        ["s", self::SIMPLE_ESCALATION_PAIR_PARAM],
      ]);
      return;
    }
    $system_data->dbcon->prepStatement("INSERT INTO configuration (param, value, is_active) VALUES (?, ?, 1)", [
      ["s", self::SIMPLE_ESCALATION_PAIR_PARAM],
      ["s", $json],
    ]);
  }
  ...
  private function setInstrumentMinimums()
  {
    ...
    $simplePairs = $this->sanitizeSimpleEscalationPairs($data["simplePairs"] ?? ($data["simplePair"] ?? []));
