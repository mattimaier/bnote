...
class ConfigurationModule {
    ...
    private function getInstrumentAdminData(): array {
        global $system_data;
        $categoriesSel = $system_data->dbcon->getSelection(
    ...
    private function deleteCategory(): array {
        ...
        $count = (int) $system_data->dbcon->colValue(
            "SELECT COUNT(*) AS cnt FROM instrument WHERE category = ?",
            "cnt",
            [['i', $id]]
        );
        if ($count > 0) {
    ...
    private function createInstrument(): array {
        ...
        if ($categoryId > 0) {
            $exists = (int) $system_data->dbcon->colValue(
                "SELECT COUNT(*) AS cnt FROM category WHERE id = ?",
                "cnt",
                [['i', $categoryId]]
            );
            if ($exists < 1) {
        ...
        $system_data->dbcon->execute(
            "INSERT INTO instrument (name, category, rank) VALUES (?, ?, ?)",
            [['s', $name], ['i', $categoryId], ['i', $rank]]
        );
        return ['success' => true];
    }
    ...
    private function updateInstrument(): array {
        ...
        $system_data->dbcon->execute(
            "UPDATE instrument SET name = ?, category = ?, rank = ? WHERE id = ?",
            [['s', $name], ['i', $categoryId], ['i', $rank], ['i', $id]]
        );
        return ['success' => true];
    }
    ...
    private function deleteInstrument(): array {
        ...
        $count = (int) $system_data->dbcon->colValue(
            "SELECT COUNT(*) AS cnt FROM contact WHERE instrument = ?",
            "cnt",
            [['i', $id]]
        );
        if ($count > 0) {
    ...
    private function saveInstrumentSections(): array {
        ...
        foreach ($sections as $section) {
            ...
            $instrumentIdsRaw = isset($section['instrument_ids']) && is_array($section['instrument_ids']) ? $section['instrument_ids'] : [];
            $instrumentIds = [];
            ...
            $targetsRaw = isset($section['concert_instrument_targets']) && is_array($section['concert_instrument_targets'])
    ...
    private function seedInstrumentDefaults(): array {
        ...
        $upsertCategory = function (string $name): int {
            global $system_data;
            $id = (int) $system_data->dbcon->colValue(
            ...
            return (int) $system_data->dbcon->colValue(
                "SELECT id FROM category WHERE name = ? LIMIT 1",
                "id",
                [['s', $name]]
            );
        ...
        $upsertInstrument = function (string $name, int $categoryId, int $rank): int {
            global $system_data;
            $id = (int) $system_data->dbcon->colValue(
                "SELECT id FROM instrument WHERE name = ? LIMIT 1",
                "id",
                [['s', $name]]
            );
            if ($id > 0) {
                $system_data->dbcon->execute(
                    "UPDATE instrument SET category = ?, rank = ? WHERE id = ?",
                    [['i', $categoryId], ['i', $rank], ['i', $id]]
                );
                return $id;
            }
            $system_data->dbcon->execute(
        ...
        $labels = [
            ...
            'bariSax' => $isDe ? 'Bariton Saxophon' : ($isEs ? 'Saxo barítono' : ($isFr ? 'Saxophone baryton' : 'Baritone Sax')),
            'piano' => 'Piano',
        ...
        $seedPresetIds = isset($presetConfig['default_seed_presets']) && is_array($presetConfig['default_seed_presets'])
    ...
    private function resolveInstrumentIdByCandidates(array $instrumentByName, array $candidates, array $categoryCandidates = []): int {
        $normalizedCategoryCandidates = array_values(array_filter(array_map(
    ...
    private function resolveLocalizedPresetName(array $presetSection, string $lang): string {
        ...
        $i18n = isset($presetSection['name_i18n']) && is_array($presetSection['name_i18n']) ? $presetSection['name_i18n'] : [];
    ...
    private function buildSectionsFromPreset(array $preset, array $instrumentByName, string $lang): array {
        ...
        foreach ($presetSections as $presetSection) {
            ...
            $instrumentIds = [];
            $slots = isset($presetSection['instrument_slots']) && is_array($presetSection['instrument_slots']) ? $presetSection['instrument_slots'] : [];
            foreach ($slots as $slot) {
                if (!is_array($slot)) {
                    continue;
                }
                $candidates = isset($slot['candidates']) && is_array($slot['candidates']) ? $slot['candidates'] : [];
                $categoryCandidates = isset($slot['category_candidates']) && is_array($slot['category_candidates']) ? $slot['category_candidates'] : [];
            ...
            $targets = [];
            $targetItems = isset($presetSection['concert_targets']) && is_array($presetSection['concert_targets']) ? $presetSection['concert_targets'] : [];
            foreach ($targetItems as $target) {
                if (!is_array($target)) {
                    continue;
                }
                $candidates = isset($target['candidates']) && is_array($target['candidates']) ? $target['candidates'] : [];
                $categoryCandidates = isset($target['category_candidates']) && is_array($target['category_candidates']) ? $target['category_candidates'] : [];
                $required = max(0, (int) ($target['required'] ?? 0));
                $instrumentId = $this->resolveInstrumentIdByCandidates($instrumentByName, $candidates, $categoryCandidates);
                if ($instrumentId < 1 || $required < 1) {
                    continue;
    ...
    private function mergeSectionsFromPresetMissingOnly(array $existingSections, array $preset, array $instrumentByName, string $lang): array {
        ...
        foreach ($presetSections as $presetSection) {
            ...
            $existingTargetsRaw = isset($section['concert_instrument_targets']) && is_array($section['concert_instrument_targets'])
                ? $section['concert_instrument_targets']
                : [];
            $targetMap = [];
            ...
            if (!isset($section['instrument_ids']) || !is_array($section['instrument_ids']) || count($section['instrument_ids']) < 1) {
                $section['instrument_ids'] = is_array($presetSection['instrument_ids'] ?? null) ? $presetSection['instrument_ids'] : [];
            }
    ...
    private function readJsonConfigParam(string $param, $fallback) {
        global $system_data;
        $raw = $system_data->dbcon->colValue(
