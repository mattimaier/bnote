...
class CommentsModule
{
  ...
  private function getUserIdByContactId($system_data, $contactId)
  {
    ...
    $rows = $system_data->dbcon->preparedQuery("SELECT id FROM user WHERE contact = ?", [["i", $contactId]]);
  ...
  private function getContactEmailById($system_data, $contactId)
  {
    ...
    $rows = $system_data->dbcon->preparedQuery("SELECT email FROM contact WHERE id = ?", [["i", $contactId]]);
    if (!$rows || count($rows) === 0 || empty($rows[0]["email"])) {
      return null;
    }
    return $rows[0]["email"];
  }
  ...
  {
    global $system_data;
    $system_data->dbcon->prepStatement("DELETE FROM comment WHERE id = ? AND author = ?", [
      ["i", $commentId],
      ["i", $userId],
    ]);
  }
