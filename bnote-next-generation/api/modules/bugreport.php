...
class BugreportModule
{
  ...
  private function send(): array
  {
    ...
    if ($screenshot !== null) {
      ...
      if ($tempScreenshotPath !== null) {
        ...
        $screenshotHtml =
          '<p><strong>Screenshot:</strong></p><p><img src="cid:bugreport-screenshot" alt="Bug report screenshot" style="max-width:100%;height:auto;border:1px solid #ddd;border-radius:8px;" /></p>';
    ...
    $message = new NextGenMailMessage([$recipient], [], $subject, $htmlBody, $textBody, "beta_bug_report", $embeds);
  ...
  private function buildHtmlBody(
    ...
    $reproLine =
      $reproducibility !== "" && $reproducibility !== "n/a"
        ? "<p><strong>Reproducibility:</strong> " .
          htmlspecialchars($reproducibility, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
          "</p>"
        : "";
    $firstSeenLine =
      $firstSeenVersion !== "" && $firstSeenVersion !== "n/a"
        ? "<p><strong>First seen version:</strong> " .
          htmlspecialchars($firstSeenVersion, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
          "</p>"
        : "";
    $expectedBlock =
      $expected !== "" && $expected !== "n/a"
        ? "<p><strong>Expected behavior</strong><br>" .
          nl2br(htmlspecialchars($expected, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8")) .
          "</p>"
        : "";
    $actualBlock =
      $actual !== "" && $actual !== "n/a"
        ? "<p><strong>Actual behavior</strong><br>" .
          nl2br(htmlspecialchars($actual, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8")) .
          "</p>"
        : "";
    ...
    return '<!doctype html><html><body style="font-family:Arial,sans-serif;line-height:1.45;color:#1f2937;">' .
      ...
      '<p style="margin:0 0 16px;"><strong>Report ID:</strong> ' .
      htmlspecialchars($reportId, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</p>" .
      "<p><strong>Severity:</strong> " .
      htmlspecialchars(strtoupper($severity), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</p>" .
      "<p><strong>Client commit:</strong> " .
      htmlspecialchars($clientBuildCommit, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</p>" .
      ...
      "<ul>" .
      "<li>Name: " .
      htmlspecialchars($repName, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</li>" .
      "<li>User ID: " .
      htmlspecialchars(strval($reporter["userId"] ?? ""), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</li>" .
      "<li>Email: " .
      htmlspecialchars((string) ($reporter["email"] ?? ""), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      "</li>" .
      "<li>Is admin: " .
      (!empty($reporter["isAdmin"]) ? "yes" : "no") .
      "</li>" .
      "</ul>" .
      "<h3>Reproduction</h3>" .
      "<p><strong>Steps to reproduce</strong><br>" .
      nl2br(htmlspecialchars($steps, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8")) .
      "</p>" .
      $expectedBlock .
      $actualBlock .
      $reproLine .
      $firstSeenLine .
      $screenshotHtml .
      '<h3>Client context (redacted)</h3><pre style="background:#f3f4f6;padding:10px;border-radius:8px;white-space:pre-wrap;">' .
      $clientJson .
      "</pre>" .
      '<h3>Network trace (redacted)</h3><pre style="background:#f3f4f6;padding:10px;border-radius:8px;white-space:pre-wrap;">' .
      $networkJson .
      "</pre>" .
      '<h3>Log trace (redacted)</h3><pre style="background:#f3f4f6;padding:10px;border-radius:8px;white-space:pre-wrap;">' .
      $logsJson .
      "</pre>" .
      '<h3>Server summary</h3><pre style="background:#f3f4f6;padding:10px;border-radius:8px;white-space:pre-wrap;">' .
      $serverJson .
      "</pre>" .
      "</body></html>";
  }
  ...
  private function buildTextBody(
    ...
    $expectedBlock = $expected !== "" && $expected !== "n/a" ? "Expected behavior:\n{$expected}\n\n" : "";
    $actualBlock = $actual !== "" && $actual !== "n/a" ? "Actual behavior:\n{$actual}\n\n" : "";
    $reproLine = $reproducibility !== "" && $reproducibility !== "n/a" ? "Reproducibility: {$reproducibility}\n" : "";
    $firstSeenLine =
      $firstSeenVersion !== "" && $firstSeenVersion !== "n/a" ? "First seen version: {$firstSeenVersion}\n" : "";
    return "Beta Bug Report\n" .
      ...
      "- Name: {$repName}\n" .
      "- User ID: " .
      strval($reporter["userId"] ?? "") .
      "\n" .
      "- Email: " .
      strval($reporter["email"] ?? "") .
      "\n" .
      "- Is admin: " .
      (!empty($reporter["isAdmin"]) ? "yes" : "no") .
      "\n\n" .
