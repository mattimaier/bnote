...
class SearchModule
{
  ...
  {
    global $system_data;
    $rehearsalsModuleId = getLegacyModuleId($system_data, LegacyModuleKey::REHEARSALS);
    $concertsModuleId = getLegacyModuleId($system_data, LegacyModuleKey::CONCERTS);
    $contactsModuleId = getLegacyModuleId($system_data, LegacyModuleKey::CONTACTS);
    $membersModuleId = getLegacyModuleId($system_data, LegacyModuleKey::MEMBERS);
  ...
  private function getConcertIdsForUser($uid)
  {
    global $system_data;
    $concerts =
      $system_data->isUserSuperUser($uid) || $this->hasConcertsModule
    ...
    try {
      ...
      if ((!$moduleType || $moduleType === "rehearsal") && $this->canSearchCategory("rehearsal")) {
        ...
        if (isset($rehearsalResult["items"])) {
          $results["rehearsals"] = $this->applyVisibilityFilter(
            "rehearsal",
            $this->filterValidItems($rehearsalResult["items"]),
          );
      ...
      if (
        (!$moduleType || $moduleType === "concert" || $moduleType === "performance") &&
        $this->canSearchCategory("concert")
      ) {
        $concertResult = $this->data->searchConcerts($query, $filters, $limit);
        if (isset($concertResult["items"])) {
          $results["concerts"] = $this->applyVisibilityFilter(
            "concert",
            $this->filterValidItems($concertResult["items"]),
          );
      ...
      if ((!$moduleType || $moduleType === "contact") && $this->canSearchCategory("contact")) {
        ...
        if (isset($contactResult["items"])) {
          $results["contacts"] = $this->applyVisibilityFilter(
            "contact",
            $this->filterValidItems($contactResult["items"]),
          );
