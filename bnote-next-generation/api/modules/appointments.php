...
class AppointmentsModule
{
  ...
  private function enrichAppointmentRow(&$row)
  {
    ...
    if ($contactId > 0) {
      global $system_data;
      $con = $system_data->dbcon->fetchRow("SELECT name, surname, email FROM contact WHERE id = ?", [
        ["i", $contactId],
      ]);
  ...
  {
    global $system_data;
    $rows = $system_data->dbcon->getSelection("SELECT `group` FROM appointment_group WHERE appointment = ?", [
      ["i", intval($id)],
    ]);
    $ids = [];
    if (is_array($rows)) {
