...
class RemindersModule
{
  ...
  private function getEscalationGroups()
  {
    ...
    global $system_data;
    ReminderAdmin::requireAdmin($system_data);
    $rows = $system_data->dbcon->getSelection("SELECT id, name FROM `group` WHERE is_active = 1 ORDER BY name", []);
    $groups = [];
    if (is_array($rows)) {
  ...
  private function getCalendarTimezone()
  {
    ...
    $stored = $system_data->dbcon->colValue("SELECT value FROM configuration WHERE param = ?", "value", [
