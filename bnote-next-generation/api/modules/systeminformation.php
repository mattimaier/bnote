...
class SysteminformationModule {
    ...
    private function getOverview(): array {
        ...
        return [
            ...
            'country' => $this->countryAlpha3ToAlpha2((string) ($system_data->getDynamicConfigParameter('default_country') ?? '')),
    ...
    private function countModules($modulesRaw): int {
        if (!is_array($modulesRaw)) return 0;
        $count = 0;
    ...
    private function countEntries(array $changelog): int {
        if (empty($changelog['entries']) || !is_array($changelog['entries'])) return 0;
        $count = 0;
    ...
    private function resolveLegacyBnoteVersion($system_data): string {
        ...
        $fileCandidates = [
            BNOTE_ROOT . '/VERSION',
            BNOTE_ROOT . '/version.txt',
            BNOTE_ROOT . '/config/version.txt',
        ];
