...
class TranslationsModule
{
  ...
  private function getPHPTranslations($langCode)
  {
    ...
    $langFile = BNOTE_ROOT . "/lang/lang_" . $langCode . ".php";
  ...
  private function getJSTranslations($langCode)
  {
    ...
    // Path: bnote-next-generation/api/modules/ -> bnote-next-generation/lang/ (go up two levels from modules/)
  ...
  private function getAllTranslations($langCode)
  {
    ...
    // Debug logging
    error_log(
      "TranslationsModule: Loaded " .
        count($phpTranslations) .
        " PHP translations, " .
        count($jsTranslations) .
        " JSON translations, " .
        count($merged) .
        " merged, " .
        count($filtered) .
        " after filtering for lang: " .
        $langCode,
    );
  ...
  private function getModuleTranslations($jsModule, $langCode)
  {
    ...
    foreach ($allTranslations as $key => $value) {
      ...
      if (strpos($key, $phpModule . "_") === 0 || strpos($key, "js." . $jsModule . ".") === 0) {
