...
class StatsModule
{
  ...
  private function periodKeyExpression(string $column, array $context): string
  {
    ...
    return $context["scope"] === "all" ? "DATE_FORMAT($expr, '%Y')" : "DATE_FORMAT($expr, '%Y-%m')";
  }
  ...
  private function resolveStatsModuleId(): int
  {
    ...
    $aliases = array_map(static fn($v) => mb_strtolower($v), $this->statsModuleAliases);
  ...
  private function monthCountMap(string $table, string $dateColumn, array $context): array
  {
    ...
    $query =
      "SELECT " .
      $this->periodKeyExpression($dateColumn, $context) .
      " as ym, COUNT(*) as cnt
                  FROM `$table`";
  ...
  private function getTopRehearsalParticipants(array $context): array
  {
    ...
    return $this->rankedParticipants(
      $this->rows($this->getSelectionSafe($query, $params, "top-rehearsal-participants")),
    );
  }
  ...
  {
    $rows = array_merge($this->getCriticalRehearsals($limit), $this->getCriticalConcerts($limit));
   ...
   */
  private function monthlyParticipationRows(
    string $eventTable,
    ...
    $query =
      "SELECT
                    " .
      $this->periodKeyExpression("e.begin", $context) .
      " as ym,
  ...
  {
    $rehearsal = $this->monthlyInvitationRows(
      "rehearsal",
      "rehearsal_contact",
      "rehearsal_user",
      "rehearsal",
  ...
  private function getActiveMemberTrend(array $labels, array $context): array
  {
    ...
    $query =
      "SELECT " .
      $this->periodKeyExpression("x.replyon", $context) .
      " as ym,
  ...
  private function getTaskCompletionLatency(array $labels, array $context): array
  {
    ...
    $query =
      "SELECT " .
      $this->periodKeyExpression("t.$completionColumn", $context) .
      " as ym,
  ...
  private function getVoteParticipationTrend(array $labels, array $context): array
  {
    ...
    foreach ($rows as $row) {
      ...
      $key = $context["scope"] === "all" ? date("Y", strtotime($end)) : date("Y-m", strtotime($end));
  ...
  private function getInstrumentGapsForConcert($cid, $minimums, $mode = "instrument")
  {
    ...
    if ($mode === "section") {
      ...
      foreach ($this->getResolvedSectionsForCoverage() as $section) {
        ...
        $targets = is_array($section["concert_instrument_targets"] ?? null)
  ...
  private function getInstrumentCoverageRisk(): array
  {
    ...
    $rehearsals = $this->getSelectionSafe(
      "SELECT id FROM rehearsal WHERE " .
        $this->normalizeDateExpression("begin") .
        " IS NOT NULL AND " .
        $this->normalizeDateExpression("begin") .
        " >= NOW() AND " .
        $this->normalizeDateExpression("begin") .
  ...
  private function getUserResponseAggregates(array $context): array
  {
    ...
    $dateFilterRehearsal =
      $context["scope"] === "all"
        ? ""
        : " AND " .
          $this->normalizeDateExpression("r.begin") .
          " IS NOT NULL AND " .
          $this->normalizeDateExpression("r.begin") .
          " >= ? AND " .
          $this->normalizeDateExpression("r.begin") .
    ...
    $dateFilterConcert =
      $context["scope"] === "all"
        ? ""
        : " AND " .
          $this->normalizeDateExpression("c.begin") .
          " IS NOT NULL AND " .
          $this->normalizeDateExpression("c.begin") .
          " >= ? AND " .
          $this->normalizeDateExpression("c.begin") .
      ...
      "SELECT
                  ...
                  FROM (
                    SELECT u.id as user_id, ru.user as response_user, ru.participate, ru.replyon,
                           ...
                           " .
      $this->normalizeDateExpression("ru.replyon") .
      " as reply_dt,
                           " .
      $this->normalizeDateExpression("r.approve_until") .
      " as approve_dt,
                           " .
      $this->normalizeDateExpression("r.begin") .
      " as event_begin_dt
                    ...
                    SELECT u.id as user_id, cu.user as response_user, cu.participate, cu.replyon,
                           ...
                           " .
      $this->normalizeDateExpression("cu.replyon") .
      " as reply_dt,
                           " .
      $this->normalizeDateExpression("c.approve_until") .
      " as approve_dt,
                           " .
      $this->normalizeDateExpression("c.begin") .
      " as event_begin_dt
   ...
   * @return array{late:int,total:int,rate:float}
   */
  private function getLateResponseStatsForType(
    string $eventTable,
    string $responseTable,
    string $fk,
    array $context,
    ...
    $query =
      "SELECT
                    COUNT(*) as total_responses,
                    SUM(CASE WHEN " .
      $this->normalizeDateExpression("r.replyon") .
      " > " .
      $this->normalizeDateExpression("e.approve_until") .
      "
                              AND " .
      $this->normalizeDateExpression("r.replyon") .
      " <= " .
      $this->normalizeDateExpression("e.begin") .
  ...
  {
    $rehearsalMix = $this->monthlyResponseMixRows(
      "rehearsal",
      "rehearsal_contact",
      "rehearsal_user",
      "rehearsal",
    ...
    $noResponseRate =
      $overallTotals["invited"] > 0 ? round(($overallTotals["pending"] / $overallTotals["invited"]) * 100, 1) : 0.0;
