...
class ParticipationModule
{
  ...
  private function saveParticipation()
  {
    ...
    if ($participate === 0 && $previous !== 0) {
      try {
        $contactId =
          (int) ($system_data->dbcon->colValue("SELECT contact FROM user WHERE id = ?", "contact", [["i", $userId]]) ??
            0);
    ...
    try {
      EscalationAlertService::triggerImmediateResolutionCheck($system_data, $eventType, $eventId, "participation_save");
    } catch (Throwable $e) {
