...
class ShareModule
{
  ...
  public function __construct()
  {
    ...
    $moduleId = getLegacyModuleId($system_data, LegacyModuleKey::SHARE);
  ...
  private function buildPublicApiUrl($params, $scriptFile = "index.php")
  {
    ...
    $isHttps =
      (!empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off") || intval($_SERVER["SERVER_PORT"] ?? 0) === 443;
  ...
  private function serveShareCard()
  {
    ...
    if ($isView && !$isRaw) {
      ...
      echo '<a href="' .
        $safeImageUrl .
        '" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:10px 14px;border-radius:10px;background:#2563eb;color:#fff;text-decoration:none;font-weight:600;width:max-content">Open image</a>';
      echo "</div></body></html>";
      exit();
    }
  ...
  private function upload()
  {
    ...
    $tmpNames = is_array($_FILES[$fileInput]["tmp_name"])
      ? $_FILES[$fileInput]["tmp_name"]
      : [$_FILES[$fileInput]["tmp_name"]];
    $errors_arr = is_array($_FILES[$fileInput]["error"])
