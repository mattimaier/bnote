...
class AuthModule
{
  ...
  private function getModules()
  {
    ...
    $mitspielerModuleId = getLegacyModuleId($system_data, LegacyModuleKey::MEMBERS);
    ...
    if (!$hasMitspielerPermission) {
      foreach ($allModules as $modId => $modRow) {
        ...
        if (
          ($nameLower === "mitspieler" || $nameLower === "members") &&
          $system_data->userHasPermission(intval($modId))
        ) {
    ...
    $wrappedEnabled = strval($system_data->getDynamicConfigParameter("wrapped_module_enabled")) === "1";
    $wrappedModuleId = intval(getLegacyModuleId($system_data, LegacyModuleKey::WRAPPED));
    ...
    if (isset($_GET["debug"]) && $_GET["debug"] === "1") {
      return [
        ...
        "debug" => [
          ...
          "dbModuleNames" => array_map(
            function ($m) {
              return $m["name"] ?? "unknown";
            },
            is_array($allDbModules) ? $allDbModules : [],
          ),
          "allModuleNames" => array_map(function ($m) {
            return $m["name"] ?? "unknown";
          }, $allModules),
  ...
  private function requestPasswordReset(): array
  {
    ...
    $resolved = NextGenPasswordReset::resolveActiveUserForReset($identifier, $this->loginData, $system_data);
    if ($resolved === null) {
      return $out;
    }
