...
class WrappedModule
{
  ...
  private function buildVibePersona(array $personal, int $year): array
  {
    ...
    $responseCompletionRate = $totalEvents > 0 ? ($totalResponses / $totalEvents) * 100.0 : 0.0;
  ...
  private function getPersonalSummary(int $userId, int $year): array
  {
    ...
    $responseRows = $this->rows($system_data->dbcon->getSelection($responseSql, array_merge($params, $params)));
    ...
    $eventRows = $this->rows($system_data->dbcon->getSelection($eventsSql, array_merge($params, $params)));
    ...
    $monthSql = "SELECT month_name, month_count
            ...
            LIMIT 0, 1";
    $monthRows = $this->rows($system_data->dbcon->getSelection($monthSql, array_merge($params, $params)));
  ...
  private function getAchievements(int $year, array $personal): array
  {
    ...
    $yesRate = floatval($personal["responses"]["yesRate"] ?? 0);
    $avgDeadlineGapHours = floatval($personal["responses"]["deadlineGapHours"] ?? 0);
    $responseCompletionRate = $totalEvents > 0 ? round(($totalResponses / $totalEvents) * 100, 1) : 0.0;
  ...
  private function getBandSummaryIfAdmin(int $year): array
  {
    ...
    $overviewRows = $this->rows($system_data->dbcon->getSelection($overviewSql, array_merge($params, $params)));
    ...
    $topSql = "SELECT c.name, COUNT(*) as score
            ...
            LIMIT 0, 5";
    $topRows = $this->rows($system_data->dbcon->getSelection($topSql, array_merge($params, $params)));
    $topResponders = [];
    ...
    $yesRate = $totalResponses > 0 ? round((intval($overview["yesResponses"] ?? 0) / $totalResponses) * 100, 1) : 0.0;
