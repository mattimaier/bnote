...
class CalendarModule
{
  ...
  public function __construct()
  {
    ...
    $uid = Auth::getUserId();
    $moduleId = getLegacyModuleId($system_data, LegacyModuleKey::CALENDAR);
  ...
  private function getEvents()
  {
    ...
    foreach ($rawEvents as $ev) {
      ...
      if ($bnoteType === "contact") {
        ...
        if ($startTrim === "" || strpos($startTrim, "0000-00-00") === 0) {
          continue;
        }
      }
  ...
  private function getRehearsalsInRange($from, $to)
  {
    ...
    if ($system_data->isUserSuperUser($uid) || $hasRehearsalsModule) {
      ...
      $rehearsalIds = array_unique(
        array_merge(is_array($userRehs) ? Database::flattenSelection($userRehs, "rehearsal") : [], $phaseRehs),
      );
  ...
  private function getConcertsInRange($from, $to)
  {
    ...
    if ($system_data->isUserSuperUser($uid) || $hasConcertsModule) {
      ...
      $phaseWhere =
        count($phases) > 0 ? "rehearsalphase IN (" . implode(",", array_fill(0, count($phases), "?")) . ")" : "0 = 1";
      $params = array_merge(array_map(fn($p) => ["i", $p], $phases), [["i", $contactId], ["s", $from], ["s", $to]]);
      $phaseQuery =
        count($phases) > 0
          ? "SELECT concert FROM rehearsalphase_concert WHERE $phaseWhere"
          : "SELECT concert FROM rehearsalphase_concert WHERE 0 = 1";
      $query =
        "SELECT DISTINCT c.id, c.title, c.begin, c.end, c.approve_until, c.notes, l.name as location_name, c.outfit " .
        "FROM concert c LEFT JOIN location l ON c.location = l.id " .
