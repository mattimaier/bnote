...
class UsersModule
{
    ...
    try {
      ...
      if ($wasActivated) {
        ...
        if ($mailCtx !== null) {
          ...
          $locale = method_exists($system_data, "getLang") ? (string) $system_data->getLang() : "en";
          $message = UserWelcomeMailBuilder::build($system_data, $locale, $mailCtx["to"], $mailCtx["firstName"]);
  ...
  private function getActivationMailContext(int $userId)
  {
    ...
    global $system_data;
    $query =
      "SELECT c.email, c.name as firstName FROM user u " .
  ...
  private function getUserIsActiveState(int $userId)
  {
    ...
    global $system_data;
    $val = $system_data->dbcon->colValue("SELECT isActive FROM user WHERE id = ?", "isActive", [["i", $userId]]);
    return intval($val) === 1;
  }
