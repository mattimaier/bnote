...
final class NextGenPublicConcertsFeedToken {
    ...
    public static function regenerate(object $db): array {
        ...
        $db->execute(
            'DELETE FROM public_concerts_feed_token WHERE scope_key = ?',
            [['s', self::SCOPE_KEY]]
        );
    ...
    public static function isValidToken(string $plainToken, object $db): bool {
        ...
        $row = $db->fetchRow(
            'SELECT id FROM public_concerts_feed_token WHERE scope_key = ? AND token_hash = ? LIMIT 1',
            [['s', self::SCOPE_KEY], ['s', $tokenHash]]
        );
    ...
    private static function insertFreshToken(object $db): array {
        ...
        $db->execute(
            'INSERT INTO public_concerts_feed_token (scope_key, token_plain, token_hash) VALUES (?, ?, ?)',
            [['s', self::SCOPE_KEY], ['s', $plain], ['s', $tokenHash]]
        );
        return [
            'plainToken' => $plain,
            'tokenHash' => $tokenHash,
