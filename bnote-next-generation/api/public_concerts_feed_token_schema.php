...
SQL;
    ...
    private static function ensureColumns(mysqli $mysqli): void {
        ...
        if (!isset($cols['scope_key'])) {
            $mysqli->query("ALTER TABLE `public_concerts_feed_token` ADD COLUMN `scope_key` varchar(32) DEFAULT 'global' AFTER `id`");
        }
        if (!isset($cols['token_plain'])) {
            $mysqli->query("ALTER TABLE `public_concerts_feed_token` ADD COLUMN `token_plain` char(64) DEFAULT NULL AFTER `scope_key`");
        }
        if (!isset($cols['token_hash'])) {
            $mysqli->query("ALTER TABLE `public_concerts_feed_token` ADD COLUMN `token_hash` char(64) DEFAULT NULL AFTER `token_plain`");
        }

        self::ensureUniqueIndex($mysqli, 'pcft_scope_key', 'ALTER TABLE `public_concerts_feed_token` ADD UNIQUE KEY `pcft_scope_key` (`scope_key`)');
        self::ensureUniqueIndex($mysqli, 'pcft_token_plain', 'ALTER TABLE `public_concerts_feed_token` ADD UNIQUE KEY `pcft_token_plain` (`token_plain`)');
        self::ensureUniqueIndex($mysqli, 'pcft_token_hash', 'ALTER TABLE `public_concerts_feed_token` ADD UNIQUE KEY `pcft_token_hash` (`token_hash`)');
    }

    private static function ensureUniqueIndex(mysqli $mysqli, string $indexName, string $ddl): void {
        $exists = false;
        if ($res = $mysqli->query("SHOW INDEX FROM `public_concerts_feed_token` WHERE Key_name = '" . $mysqli->real_escape_string($indexName) . "'")) {
