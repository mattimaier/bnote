...
final class CalendarSubscriptionTokenSchema
{
  ...
  private static function ensureColumns(mysqli $mysqli): void
  {
    ...
    if (!isset($cols["token_plain"])) {
      $mysqli->query(
        "ALTER TABLE `calendar_subscription_token` ADD COLUMN `token_plain` char(64) DEFAULT NULL AFTER `user_id`",
      );
    }
    if (!isset($cols["token_hash"])) {
      $mysqli->query(
        "ALTER TABLE `calendar_subscription_token` ADD COLUMN `token_hash` char(64) DEFAULT NULL AFTER `token_plain`",
      );
    }
    ...
    self::ensureUniqueIndex(
      $mysqli,
      "cst_user_id",
      "ALTER TABLE `calendar_subscription_token` ADD UNIQUE KEY `cst_user_id` (`user_id`)",
    );
    self::ensureUniqueIndex(
      $mysqli,
      "cst_token_plain",
      "ALTER TABLE `calendar_subscription_token` ADD UNIQUE KEY `cst_token_plain` (`token_plain`)",
    );
    self::ensureUniqueIndex(
      $mysqli,
      "cst_token_hash",
      "ALTER TABLE `calendar_subscription_token` ADD UNIQUE KEY `cst_token_hash` (`token_hash`)",
    );
  }
  ...
  private static function ensureUniqueIndex(mysqli $mysqli, string $indexName, string $ddl): void
  {
    ...
    if (
      $res = $mysqli->query(
        "SHOW INDEX FROM `calendar_subscription_token` WHERE Key_name = '" .
