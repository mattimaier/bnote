...
final class NextGenPublicEventVisibility {
    ...
    public static function setPublished(object $db, string $otype, int $oid, bool $isPublished, ?int $publishedByUserId = null): bool {
        $otype = self::normalizeOtype($otype);
        if ($otype === '' || $oid < 1) {
            return false;
        }
        ...
        if ($isPublished) {
            ...
            if ($by === null) {
                $db->execute(
                    'UPDATE nextgen_public_event_visibility SET published_by = NULL WHERE otype = ? AND oid = ?',
                    [['s', $otype], ['i', $oid]]
                );
            }
            return true;
    ...
    public static function deleteForEvent(object $db, string $otype, int $oid): void {
        ...
        $db->execute(
            'DELETE FROM nextgen_public_event_visibility WHERE otype = ? AND oid = ?',
            [['s', $otype], ['i', $oid]]
        );
    }
