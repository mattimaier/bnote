...
final class ParticipationMagicApply
{
  ...
  {
    if ($eventType === "R") {
      $row = $db->fetchRow("SELECT 1 FROM rehearsal_contact WHERE rehearsal = ? AND contact = ? LIMIT 1", [
        ["i", $eventId],
        ["i", $contactId],
      ]);
      return is_array($row);
    }
    $row = $db->fetchRow("SELECT 1 FROM concert_contact WHERE concert = ? AND contact = ? LIMIT 1", [
      ["i", $eventId],
      ["i", $contactId],
    ]);
    return is_array($row);
