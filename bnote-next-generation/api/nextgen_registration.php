...
class NextGenRegistration {
    ...
    public static function execute(array $body): array {
        ...
        $db->execute(
            'INSERT INTO contact_group (contact, `group`) VALUES (?, ?)',
            [['i', $cid], ['i', (int) $defaultGroup]]
        );

        $userQ = 'INSERT INTO user (login, password, isActive, contact) VALUES (?, ?, 0, ?)';
        $uid = $db->prepStatement($userQ, [
            ['s', $email],
            ['s', $passwordEnc],
            ['i', $cid],
        ]);
        ...
        $mods = array_values(array_filter(array_map(static function ($m) {
            $t = trim((string) $m);
            return $t === '' ? null : $t;
        }, $mods)));
        if (count($mods) < 1) {
        ...
        if ($system_data->autoUserActivation()) {
            if ($system_data->inDemoMode()) {
                ...
                $linkurl = $system_data->getSystemURL() . '/src/export/useractivation.php?uid=' . $uid . '&email=' . rawurlencode($email);
                ...
                $message = Lang::txt('LoginController_register.message_1')
                    . '<a href="' . $linkurl . '">' . Lang::txt('LoginController_register.message_2') . '</a>';
