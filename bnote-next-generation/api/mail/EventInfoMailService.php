...
final class EventInfoMailService {
    ...
    public static function previewHtml(
        ...
        $message = self::buildMessage($system_data, $otype, $oid, $locale, $recipientIds, $manualEmails, $subject, $body, $senderName);
    ...
    private static function loadEventData($system_data, string $otype, int $oid, string $locale): array {
        ...
        if ($dtBegin !== null) {
            $detailLines[] = [
                ...
                'value' => $sameDay ? MailLocaleDateTime::formatTimeShort($dtBegin, $locale) : MailLocaleDateTime::formatDateTimeShort($dtBegin, $locale),
            ];
        }
        if ($dtEnd !== null) {
            $detailLines[] = [
                'label' => MailI18n::t('mail.eventInfo.detailEnd', $locale),
                'value' => $sameDay ? MailLocaleDateTime::formatTimeShort($dtEnd, $locale) : MailLocaleDateTime::formatDateTimeShort($dtEnd, $locale),
            ];
        }
        ...
        if ($title === '') {
            if ($locationName !== '') {
                ...
                $title = $otype === 'R' ? MailI18n::t('js.event.rehearsal', $locale) : MailI18n::t('js.event.performance', $locale);
            }
        }
        ...
        return [
            'otype' => $otype,
            'title' => $title,
            'typeLabel' => $otype === 'R' ? MailI18n::t('js.event.rehearsal', $locale) : MailI18n::t('js.event.performance', $locale),
            'dateLine' => $dateLine,
     ...
     * @return list<string>
     */
    private static function resolveOutgoingRecipients($system_data, string $otype, int $oid, array $recipientIds, array $manualEmails): array {
