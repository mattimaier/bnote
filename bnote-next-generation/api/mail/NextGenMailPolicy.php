...
final class NextGenMailPolicy
{
  ...
  public static function contactTransactionalMailDenyReason($system_data, int $contactId): ?string
  {
    ...
    $activeUid = $db->colValue("SELECT id FROM user WHERE contact = ? AND isActive = 1", "id", [["i", $contactId]]);
    if ($activeUid !== null) {
    ...
    $anyUid = $db->colValue("SELECT id FROM user WHERE contact = ? LIMIT 1", "id", [["i", $contactId]]);
    if ($anyUid !== null) {
