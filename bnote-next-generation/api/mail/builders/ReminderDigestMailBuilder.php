...
final class ReminderDigestMailBuilder
{
  ...
  public static function build(
    ...
    $headlineKey = isset($options["headlineKey"])
      ? (string) $options["headlineKey"]
      : "mail.shell.headlineReminderDigest";
    ...
    if ($recipientFirstName !== null && trim($recipientFirstName) !== "") {
      $intro .=
        '<p class="em-lead" style="margin:0 0 8px;">' .
        MailI18n::interpolate(MailI18n::t("mail.commentDiscussion.greetingHi", $locale), [
          "firstName" => htmlspecialchars(trim($recipientFirstName), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8"),
        ]) .
        "</p>";
    }
    $intro .=
      '<p class="em-lead" style="margin:0 0 18px;">' .
    ...
    return new NextGenMailMessage($to, $bcc, $subject, $html, $plain, $templateKey, MailAssets::defaultLogoEmbeds());
  }
  ...
  private static function sectionHtml(string $locale, string $titleKey, array $items, bool $withTraffic): string
  {
    ...
    return '<div style="margin:0 0 20px;">' .
      '<p class="em-section-title" style="margin:0 0 10px;">' .
      $title .
      "</p>" .
  ...
  private static function rowHtml(string $locale, array $item, bool $withTraffic): string
  {
    ...
    if ($withTraffic && ($otype === "R" || $otype === "C")) {
      ...
      $participationLabel = htmlspecialchars(
      ...
      $hasDifferentTitle =
        trim($titleRaw) !== "" &&
        strtolower(trim($titleRaw)) !== strtolower(trim(self::eventTypeLabel($locale, $otype)));
      $titleLine = "";
      if ($hasDifferentTitle && $eventDate !== $titleRaw) {
        $titleLine =
          '<div style="margin-top:2px;font-size:14px;line-height:1.35;font-weight:600;color:' .
      ...
      $leftMain =
        '<div style="font-size:16px;line-height:1.3;font-weight:700;color:' .
        htmlspecialchars(MailDesignTokens::get("textPrimary"), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        ';">' .
        $eventDateEsc .
        "</div>" .
        $titleLine .
        '<div style="margin-top:8px;font-size:14px;line-height:1.35;color:' .
        htmlspecialchars(MailDesignTokens::get("textMuted"), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        ';">' .
        self::metaIconClock() .
        ...
        "</span>" .
        "</div>" .
        '<div style="margin-top:4px;font-size:14px;line-height:1.35;color:' .
        htmlspecialchars(MailDesignTokens::get("textMuted"), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        ';">' .
        self::metaIconMapPin() .
        '<span style="vertical-align:middle;margin-left:6px;">' .
      ...
      $rightMain =
        '<div style="text-align:right;font-size:14px;line-height:1.3;font-weight:600;color:' .
        htmlspecialchars(MailDesignTokens::get("textMuted"), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        ';margin-bottom:8px;">' .
        $participationLabel .
        "</div>" .
      ...
      return "<tr>" .
        ...
        '<td width="42" valign="top" style="padding-right:10px;">' .
        $icon .
        "</td>" .
    ...
    $linkStart =
      $primaryHref !== ""
        ? '<a class="em-link" href="' . $primaryHref . '" style="text-decoration:none;color:inherit;">'
        : "";
    ...
    return "<tr>" .
      ...
      "</td>" .
      '<td valign="top">' .
      '<div style="font-size:16px;line-height:1.3;font-weight:600;">' .
      $title .
      " " .
      $statusPill .
      "</div>" .
  ...
  private static function eventTypeBadgeHtml(string $locale, string $otype): string
  {
    ...
    $badgeBg =
      (string) ($acc["badge_bg"] ?? MailEntityColors::mixWithWhite(MailEntityColors::solidHex($entityKey), 0.12));
    $badgeBorder =
      (string) ($acc["badge_border"] ?? MailEntityColors::mixWithWhite(MailEntityColors::solidHex($entityKey), 0.24));
    ...
    return '<span style="display:inline-flex;align-items:center;padding:2px 8px;border-radius:8px;border:1px solid ' .
      ...
      ";background-color:" .
      htmlspecialchars($badgeBg, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ";font-size:12px;font-weight:500;color:" .
      htmlspecialchars($badgeText, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';">' .
  ...
  private static function metaIconClock(): string
  {
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="' .
      $stroke .
  ...
  private static function statusPillHtml(string $label, ?string $rawStatus): string
  {
    ...
    return '<span style="display:inline-block;margin-left:6px;padding:2px 10px;border-radius:8px;border:1px solid ' .
      ...
      ";background-color:" .
      htmlspecialchars($bg, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
  ...
  private static function iconBubbleForType(string $otype): string
  {
    ...
    return '<div style="width:36px;height:36px;border-radius:' .
      $bubbleRadius .
      ";background:" .
      htmlspecialchars(
        (string) ($acc["icon_bg"] ?? MailDesignTokens::get("primary")),
        ENT_QUOTES | ENT_SUBSTITUTE,
        "UTF-8",
      ) .
  ...
  private static function participationLightsHtml(int $participation, array $urls, bool $allowMaybe): string
  {
    ...
    foreach ($choices as $index => $choice) {
      ...
      $iconWrap =
        ...
        "px;height:" .
        $sizeStr .
        'px;border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;">' .
        ...
        "px;height:" .
        $sizeStr .
        'px;padding:0;line-height:0;font-size:0;mso-line-height-rule:exactly;">' .
      ...
      if ($href !== "") {
        $dot =
          ...
          'aria-label="' .
          htmlspecialchars($choice, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
          '">' .
          $dotInner .
          "</a>";
      }
      $isLast = $index === $count - 1;
      $cells[] = '<td style="padding:0' . ($isLast ? "" : " 10px 0 0") . ';">' . $dot . "</td>";
    }
  ...
  private static function trafficIcon(string $choice, string $stroke, int $sizePx): string
  {
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" width="' .
      ...
      htmlspecialchars($stroke, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
  ...
  private static function buildPlain(
    ...
    $plain .= self::plainSection(
      $locale,
      MailI18n::t("mail.reminder.sectionEventsPendingResponse", $locale),
      $eventsPendingResponse,
    );
