...
final class EventInfoMailBuilder
{
  ...
  public static function build($system_data, string $locale, array $ctx, array $to, array $bcc): NextGenMailMessage
  {
    ...
    $headline = htmlspecialchars(
      MailI18n::t("mail.shell.headlineEventInfo", $locale),
    ...
    if (is_array($detailLines) && count($detailLines) > 0) {
      foreach ($detailLines as $row) {
        ...
        if ($hrefRaw !== "") {
          ...
          $value =
            '<a class="em-link" href="' . $href . '" target="_blank" rel="noopener noreferrer">' . $value . "</a>";
        }
        $detailsRows .=
          '<tr><td style="padding:0 0 8px;vertical-align:top;width:34%;font-weight:600;">' .
          $label .
          "</td>" .
    ...
    $detailsBlock = "";
    if ($detailsRows !== "") {
      $detailsTitle = htmlspecialchars(
        MailI18n::t("mail.eventInfo.detailsTitle", $locale),
        ENT_QUOTES | ENT_SUBSTITUTE,
        "UTF-8",
      );
      $detailsBlock =
    ...
    $inner = $eventCard . $detailsBlock . '<div style="margin:0 0 10px;">' . $custom["html"] . "</div>";

    $ctaHref = $eventLinkRaw !== "" ? $eventLinkEsc : null;
    $ctaLabel =
      $eventLinkRaw !== ""
        ? htmlspecialchars(MailI18n::t("mail.eventInfo.ctaOpen", $locale), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8")
        : null;
    ...
    return new NextGenMailMessage($to, $bcc, $subject, $html, $plain, "event_info", MailAssets::defaultLogoEmbeds());
  }
   ...
   * @param list<string> $bcc
   */
  public static function buildForPreview(
    $system_data,
    string $locale,
    array $ctx,
    array $to,
  ...
  public static function renderEditorBody(string $value): array
  {
    ...
    if (!is_array($parsed) || !isset($parsed["blocks"]) || !is_array($parsed["blocks"])) {
      return [
        "html" =>
          '<p style="margin:0;">' .
          nl2br(htmlspecialchars($trimmed, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8"), false) .
          "</p>",
        "text" => $trimmed,
      ];
    }
    ...
    foreach ($parsed["blocks"] as $block) {
      ...
      if ($type === "header") {
        ...
        $htmlParts[] =
          "<h" .
          $level .
          ' style="margin:0.83em 0;font-size:' .
          $fontSize .
          'em;line-height:1.25;font-weight:500;">' .
          $txt .
          "</h" .
          $level .
          ">";
      ...
      if ($type === "code") {
        ...
        $htmlParts[] =
          '<pre class="em-surface-pre" style="margin:0 0 1em;padding:10px 12px;border-radius:8px;overflow:auto;"><code style="font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,Liberation Mono,monospace;font-size:13px;line-height:1.45;">' .
          $codeEsc .
          "</code></pre>";
        $textParts[] = $codeRaw;
        continue;
      ...
      if ($type === "quote") {
        ...
        $htmlParts[] =
          '<blockquote class="em-surface-quote" style="margin:0 0 1em;padding:8px 12px;font-style:italic;">' .
          $txt .
          "</blockquote>";
  ...
  {
    return preg_replace_callback(
      "/<(\/?)(b|strong|i|em|u|s|mark|code|br)\b[^>]*>/i",
      function (array $matches): string {
  ...
  private static function styleInlineTags(string $html): string
  {
    $out =
      preg_replace(
        "/<code>/i",
        '<code class="em-surface-code" style="font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,Liberation Mono,monospace;font-size:0.92em;">',
        $html,
      ) ?? $html;
  ...
  private static function renderEditorList(array $items, string $tag, int $depth): array
  {
    ...
    foreach ($items as $item) {
      ...
      $childRendered =
        count($children) > 0
          ? self::renderEditorList($children, $childTag, $depth + 1)
          : ["html" => "", "textLines" => []];
    ...
    $marginStyle =
      $depth > 0
        ? "margin:0.35em 0 0 1.5em;padding:0;list-style-type:" . $listStyle . ";"
        : "margin:0 0 1em 1.5em;padding:0;list-style-type:" . $listStyle . ";";
    return [
