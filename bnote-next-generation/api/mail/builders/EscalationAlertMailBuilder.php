...
final class EscalationAlertMailBuilder
{
  ...
  public static function build(
    ...
    $headline = htmlspecialchars(
      MailI18n::t("mail.shell.headlineEscalationAlert", $locale),
    ...
    $eventTypeLabel =
      $otypeNorm === "C" ? MailI18n::t("js.event.performance", $locale) : MailI18n::t("js.event.rehearsal", $locale);
    ...
    $intro =
      '<p class="em-lead" style="margin:0 0 10px;">' .
    ...
    $entityHeader = MailEntityCard::entityHeaderHtml(
      [
        ...
        "badge_bg" => MailEntityColors::mixWithWhite(MailEntityColors::solidHex($entityKey), 0.1),
    ...
    if (count($reasons) > 0) {
      ...
      foreach ($reasons as $reason) {
        $reasonsHtml .=
          '<li style="margin:0 0 4px;">' . htmlspecialchars($reason, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") . "</li>";
      }
      $reasonsHtml .= "</ul>";
    }
    ...
    if (is_array($participationCounts) && (int) ($participationCounts["invited_users"] ?? 0) > 0) {
      $plain .=
        ...
        ": " .
        MailI18n::t("js.common.yes", $locale) .
        " " .
        (int) ($participationCounts["yes"] ?? 0) .
        ", " .
        MailI18n::t("js.participation.maybe", $locale) .
        " " .
        (int) ($participationCounts["maybe"] ?? 0) .
        ", " .
        MailI18n::t("js.common.no", $locale) .
        " " .
        (int) ($participationCounts["no"] ?? 0) .
        ", " .
        MailI18n::t("mail.escalation.pendingLabel", $locale) .
        " " .
        (int) ($participationCounts["pending_users"] ?? 0);
    }
    if (count($gaps) > 0) {
  ...
  private static function alertBannerHtml(string $locale, string $urgency, string $urgencyLabelEsc): string
  {
    ...
    return '<div style="margin:0 0 12px;padding:10px 12px;border:1px solid ' .
      htmlspecialchars($border, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ";background:" .
      htmlspecialchars($bg, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';border-radius:10px;">' .
      '<span class="em-alert-chip" style="display:inline-flex;align-items:center;gap:6px;padding:3px 8px;border-radius:999px;border:1px solid ' .
      htmlspecialchars($border, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ";font-size:12px;line-height:1.2;font-weight:700;color:" .
      htmlspecialchars($text, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';">' .
      '<span style="display:inline-flex;align-items:center;justify-content:center;color:' .
      htmlspecialchars($text, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';">' .
      $icon .
      "</span>" .
      ...
      "</span>" .
      "</span>" .
      '<span style="margin-left:8px;font-size:13px;font-weight:600;color:' .
      htmlspecialchars($text, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';">' .
  ...
  private static function participationBarHtml(string $locale, ?array $counts): string
  {
    ...
    foreach ($segments as $i => $seg) {
      ...
      $parts[] =
        '<div style="height:32px;display:flex;align-items:center;justify-content:center;color:#ffffff;font-size:12px;font-weight:600;flex-shrink:0;' .
        $radius .
        "width:" .
        number_format($pct, 6, ".", "") .
        "%;background-color:" .
        htmlspecialchars($colors[(string) $seg["key"]] ?? "#9aa0a6", ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        ';">' .
    ...
    return '<div style="margin:0 0 14px;">' .
      ...
      ";background:" .
      htmlspecialchars(MailDesignTokens::get("surfaceBorder", "#e5e7eb"), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
      ';">' .
      implode("", $parts) .
      "</div>" .
      "</div>";
