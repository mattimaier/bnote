...
final class PasswordResetMailBuilder
{
  ...
  public static function build($system_data, string $locale, string $toEmail, string $resetUrl): NextGenMailMessage
  {
    ...
    $expiry = htmlspecialchars(
      MailI18n::t("mail.passwordReset.expiryNote", $locale),
    ...
    $bodyHtml =
      ...
      $intro .
      "</p>" .
      '<p class="em-text-secondary" style="margin:0 0 8px;font-size:14px;">' .
      $expiry .
      "</p>";

    if ($resetUrl === "") {
      $bodyHtml .=
        '<p class="em-muted" style="margin:16px 0 0;font-size:13px;">' .
        htmlspecialchars(MailI18n::t("mail.passwordReset.noDeepLink", $locale), ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8") .
        "</p>";
    }

    $headline = htmlspecialchars(
      MailI18n::t("mail.shell.headlinePasswordReset", $locale),
      ENT_QUOTES | ENT_SUBSTITUTE,
      "UTF-8",
    );
