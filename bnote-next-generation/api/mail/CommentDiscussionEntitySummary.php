...
final class CommentDiscussionEntitySummary
{
  ...
  private static function loadRehearsal(int $oid, $system_data, string $locale)
  {
    ...
    if (!empty($r["conductor"]) && $system_data && isset($system_data->dbcon)) {
      $row = $system_data->dbcon->fetchRow("SELECT name, surname FROM contact WHERE id = ?", [
        ["i", (int) $r["conductor"]],
      ]);
      if (is_array($row)) {
  ...
  private static function loadConcert(int $oid, $system_data, string $locale)
  {
    ...
    if (!empty($c["location"]) && isset($system_data->dbcon)) {
      $row = $system_data->dbcon->fetchRow("SELECT name FROM location WHERE id = ?", [["i", (int) $c["location"]]]);
      if (is_array($row)) {
  ...
  private static function loadVote(int $oid, $system_data, string $locale)
  {
    ...
    $finished =
      isset($v["is_finished"]) && ($v["is_finished"] == "1" || $v["is_finished"] === 1 || $v["is_finished"] === true);
    $badgeLabel = $finished ? MailI18n::t("js.votes.finished", $locale) : MailI18n::t("js.votes.active", $locale);
