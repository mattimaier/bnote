...
final class ReminderInboxSource
{
  ...
  public function getAllInboxItemsForUser(int $userId): array
  {
    ...
    for ($i = 1; $i < count($allRehearsals); $i++) {
      ...
      $items[] = [
        ...
        "oid" => (int) $r["id"],
        "title" =>
          Lang::txt("StartData_inboxItems.rehearsalOn") . " " . Data::convertDateFromDb((string) ($r["begin"] ?? "")),
    ...
    for ($i = 1; $i < count($allConcerts); $i++) {
      ...
      $items[] = [
        ...
        "oid" => (int) ($c["id"] ?? 0),
        "title" =>
          Lang::txt("StartData_inboxItems.concertOn") . " " . Data::convertDateFromDb((string) ($c["begin"] ?? "")),
  ...
  public function formatInboxItems(array $inboxItems): array
  {
    ...
    foreach ($inboxItems as $item) {
      ...
      $dueDateIso =
        is_string($dueDateRaw) && trim($dueDateRaw) !== "" && strlen(trim($dueDateRaw)) >= 10
          ? trim($dueDateRaw)
          : null;
  ...
  public function buildDigestSummaryForUser(int $userId, array $cfg): array
  {
    ...
    foreach ($all as $item) {
      ...
      if ($otype === "R" || $otype === "C") {
        $eventTs =
          isset($item["eventBegin"]) && is_string($item["eventBegin"]) ? strtotime($item["eventBegin"]) : false;
        if ($eventTs === false || $eventTs <= 0) {
          $eventTs =
            isset($item["replyUntil"]) && is_string($item["replyUntil"]) ? strtotime($item["replyUntil"]) : false;
        }
  ...
  {
    $optRows = $this->systemData->dbcon->getSelection("SELECT COUNT(*) as cnt FROM vote_option WHERE vote = ?", [
      ["i", $voteId],
    ]);
  ...
  private function loadParticipation(string $otype, int $eventId, int $userId): int
  {
    ...
    if (
      !is_array($row) ||
      !array_key_exists("participate", $row) ||
      $row["participate"] === null ||
      $row["participate"] === ""
    ) {
      return -1;
    }
