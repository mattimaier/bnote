...
final class MailEntityColors
{
  ...
  public static function statusHex(string $status): string
  {
    ...
    if (
      isset($cfg["statuses"]) &&
      is_array($cfg["statuses"]) &&
      isset($cfg["statuses"][$statusKey]) &&
      is_array($cfg["statuses"][$statusKey])
    ) {
  ...
  private static function oklchToHex(string $raw): ?string
  {
    ...
    $c = (float) $m[2];
    $h = (float) $m[3];
    $l = str_ends_with($lRaw, "%") ? ((float) rtrim($lRaw, "%")) / 100.0 : (float) $lRaw;
    $l = max(0.0, min(1.0, $l));
    $rad = deg2rad($h);
    $a = $c * cos($rad);
    $b = $c * sin($rad);
    ...
    $bLin = -(0.0041960863 * $l3 - 0.7034186147 * $m3 + 1.707614701 * $s3);
  ...
  public static function commentDiscussionCardAccents(string $entityKey): array
  {
    ...
    return [
      "icon_bg" => $icon_bg,
      "badge_bg" => self::mixWithWhite($icon_bg, 0.1),
