...
if (!PublicConcertsFeedRateLimit::consume()) {
  ...
  echo json_encode(
    ["success" => false, "error" => "public_concerts_feed_rate_limited"],
    JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES,
  );
  exit();
}
...
if (!$feedEnabled) {
  ...
  echo json_encode(
    ["success" => false, "error" => "public_concerts_feed_disabled"],
    JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES,
  );
  exit();
}
...
if (!NextGenPublicConcertsFeedToken::isValidToken($tokenRaw, $db)) {
  http_response_code(403);
  echo json_encode(
    ["success" => false, "error" => "public_concerts_feed_token_invalid"],
    JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES,
  );
  exit();
}
