...
final class CalendarIcsFeedBuilder {
    ...
    private function createEvent(
        ...
        $organizerName = trim(is_scalar($organizerRaw) || (is_object($organizerRaw) && method_exists($organizerRaw, '__toString')) ? (string) $organizerRaw : '');
        if ($organizerName === '') {
            $organizerName = 'BNote';
        }
    ...
    private function getConcerts(): array {
        ...
        $directRows = $this->db->getSelection(
    ...
    private function getConcertsFromPhases(): array {
        ...
        [$inSql, $params] = $this->buildInParams(array_map('intval', $phases));
        return $this->rows($this->db->getSelection(
    ...
    private function buildEntityUrl(string $type, int $id): string {
        ...
        return $this->appBaseUrl . '/entity?' . http_build_query([
            'type' => $type,
            'id' => (string) $id,
        ]);
    }
