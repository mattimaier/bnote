...
final class NextGenCalendarSubscriptionToken
{
  ...
  public static function getOrCreateForUser(int $userId, object $db): array
  {
    ...
    $row = $db->fetchRow("SELECT token_plain, token_hash FROM calendar_subscription_token WHERE user_id = ? LIMIT 1", [
      ["i", $userId],
    ]);
  ...
  public static function regenerateForUser(int $userId, object $db): array
  {
    ...
    $db->execute("DELETE FROM calendar_subscription_token WHERE user_id = ?", [["i", $userId]]);
    return self::insertFreshToken($userId, $db);
  }
  ...
  private static function insertFreshToken(int $userId, object $db): array
  {
    ...
    $db->execute("INSERT INTO calendar_subscription_token (user_id, token_plain, token_hash) VALUES (?, ?, ?)", [
      ["i", $userId],
      ["s", $plain],
      ["s", $tokenHash],
    ]);
