...
class Api {
    constructor() {
        // API lives at /bnote-next-generation/api/index.php. Compute base path up to and including /bnote-next-generation.
        ...
        if (bnoteNextGenSlashIndex !== -1) {
            // Found '/bnote-next-generation/' - include up to and including the trailing slash
            basePath = pathname.substring(0, bnoteNextGenSlashIndex + '/bnote-next-generation/'.length);
        } else if (bnoteNextGenIndex !== -1) {
            // Found '/bnote-next-generation' without trailing slash (e.g., /bnote/bnote-next-generation/login.html)
            // Include up to and including 'bnote-next-generation', then add trailing slash
            const endPos = bnoteNextGenIndex + '/bnote-next-generation'.length;
            basePath = pathname.substring(0, endPos) + '/';
        } else if (bnoteNextSlashIndex !== -1) {
            basePath = pathname.substring(0, bnoteNextSlashIndex + 11); // through /BNoteNext/ (backward compat)
        } else if (bnoteNextIndex !== -1) {
            basePath = pathname.substring(0, bnoteNextIndex + 10); // through /BNoteNext (backward compat)
        } else if (nextSlashIndex !== -1) {
            basePath = pathname.substring(0, nextSlashIndex + 6); // through /next/ (backward compat)
        } else if (nextIndex !== -1) {
            basePath = pathname.substring(0, nextIndex + 5);      // through /next
        } else {
            const pathParts = pathname.split('/').filter(p => p && p !== '');
            // Try bnote-next-generation first
            const bnoteNextGenPos = pathParts.indexOf('bnote-next-generation');
            if (bnoteNextGenPos !== -1) {
                basePath = '/' + pathParts.slice(0, bnoteNextGenPos + 1).join('/');
            } else {
                // Fallback to old 'next' for backward compatibility
    ...
    async request(module, action, data = null, params = {}) {
        ...
        // Add params to URL (handle nested objects like filter[date_year])
        Object.keys(params).forEach(key => {
            const value = params[key];
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                // Handle nested objects (e.g., filter: { date_year: 2026 })
                Object.keys(value).forEach(nestedKey => {
                    url.searchParams.set(`${key}[${nestedKey}]`, value[nestedKey]);
                });
            } else {
...
const SearchApi = {
    search: (query, filters = {}, limit = 5) => {
        ...
        return api.get('search', null, params);
    }
};
