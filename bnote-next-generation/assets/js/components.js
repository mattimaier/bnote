...
const Components = {
    ...
    renderTopbar(options = {}) {
        ...
        const searchPlaceholderI18n = options.searchPlaceholderI18n || 'js.search.placeholder';
        ...
        const searchPlaceholder = searchPlaceholderI18n && typeof i18n !== 'undefined' && i18n.t 
            ...
            : (options.searchPlaceholder || 'Search everything...');
            ...
            <div class="flex h-16 items-center justify-between px-4 lg:px-6 gap-4">
                ...
                <div class="flex flex-1 w-full gap-2">
                    <div class="relative flex-1 w-full">
                        ...
                        <button id="search-clear-btn" class="hidden absolute right-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors rounded-md flex items-center justify-center"
                            title="Clear search" aria-label="Clear search">
                            <i data-lucide="x" class="h-4 w-4"></i>
                        </button>
                    </div>
                    <button id="search-button" class="hidden h-9 px-4 bg-primary text-primary-foreground hover:bg-primary/90 rounded-md text-sm font-medium transition-colors flex items-center gap-2"
                        title="Search" aria-label="Search">
                        <i data-lucide="search" class="h-4 w-4"></i>
                        <span class="hidden sm:inline">${this.escapeHtml(t('js.search.button') || 'Search')}</span>
                    </button>
                </div>
    ...
    renderSidebar() {
        ...
        console.log('Components.renderSidebar: logoPath =', logoPath);
            ...
            <div id="sidebar-header" class="flex items-center h-16 px-4 lg:px-6">
                <div id="sidebar-brand" class="flex items-center gap-3">
                    <div
                        ...
                        <img src="${logoPath}" alt="BNote" class="h-5 w-5" style="filter: brightness(0) saturate(100%) invert(58%) sepia(95%) saturate(2878%) hue-rotate(195deg) brightness(102%) contrast(101%);" />
                    </div>
