...
function integrationMailFeedback(
  ...
  if (sent > 0) {
    ...
    return {
      text: interpolation(
        label !== "js.contacts.integrationMailSent" ? label : "Integration email sent ({sent}/{attempted}).",
        {
          sent: String(sent),
  ...
  if (attempted > 0) {
    ...
    return {
      text: interpolation(
        label !== "js.contacts.integrationMailNotSent" ? label : "Integration email not sent ({sent}/{attempted}).",
        {
          sent: String(sent),
...
export default function ContactsIntegrationPage() {
  ...
  const loadBundle = useCallback(async () => {
    ...
    if (mode === "remove" && removeContactId <= 0) {
      setBundle(null);
      setError(
        t("js.contacts.integrationRemoveContactMissing") !== "js.contacts.integrationRemoveContactMissing"
  ...
  const toggleAll = (setter: Dispatch<SetStateAction<Set<number>>>) => (ids: number[], checked: boolean) => {
    setter((prev) => {
      const n = new Set(prev);
  ...
  const save = async (skipRemoveConfirm = false) => {
    ...
    if (mode === "remove" && selReh.size < 1 && selCon.size < 1 && selPhases.size < 1 && selVotes.size < 1) {
      showToast(
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((m) => {
      const parts = [m.name, m.surname, m.nickname, m.email, m.instrumentname, m.label]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const sortedRemoveEvents = useMemo(() => {
    return [...removeEvents].sort((a, b) => {
      ...
      const aTitle = a.eventType === "concert" ? (a.title ?? t("js.event.performance")) : t("js.event.rehearsal");
      const bTitle = b.eventType === "concert" ? (b.title ?? t("js.event.performance")) : t("js.event.rehearsal");
  ...
  const allVisibleEventsSelected = useMemo(() => {
    ...
    return visibleRehearsalIds.every((id) => selReh.has(id)) && visibleConcertIds.every((id) => selCon.has(id));
  ...
  const groupPickerOptions = useMemo(() => groups.map((g) => ({ id: g.id, name: g.name })), [groups]);

  const groupPickerValue = useMemo(() => {
    if (groups.length === 0) return 0;
    const fromState = selectedGroup != null ? Number.parseInt(selectedGroup, 10) : Number.NaN;
    ...
    <div className={`${PAGE_CONTENT_CLASS} pb-24 md:pb-8`}>
      ...
      {mode === "add" ? (
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:gap-3">
          <span className="text-sm text-base-content/70 sm:shrink-0 sm:pt-2.5">
            {t("js.contacts.integrationGroupPicker")}
          </span>
          ...
          <section className="rounded-lg border border-base-300 bg-base-100 p-3">
            ...
            <ResponsiveTable<RemoveEventRow, "title" | "begin" | "status">
              ...
              renderMobileRow={(row) => {
                ...
                const title =
                  row.eventType === "concert" ? (row.title ?? t("js.event.performance")) : t("js.event.rehearsal");
                  ...
                  <div className="flex items-center gap-2 pr-2">
                    <label className="checkbox checkbox-sm checkbox-primary shrink-0 ml-1">
                      <input
                        ...
                        onChange={() =>
                          row.eventType === "rehearsal" ? toggle(setSelReh)(row.id) : toggle(setSelCon)(row.id)
                        }
                      />
                    </label>
                  ...
                  <tr className="border-b border-base-300 bg-base-200/50">
                    ...
                    <th className="p-3 text-left font-semibold">
                      <button
                        ...
                        onClick={() => handleRemoveEventSort("status")}
                      >
                  ...
                  {sortedRemoveEvents.map((row) => {
                    ...
                    return (
                      <tr key={`${row.eventType}-${row.id}`} className="border-b border-base-300 hover:bg-base-200/50">
                        ...
                        <td className="p-3">
                          {[
                            formatEventDate(row.begin, lang, tba),
                            formatEventTime(row.begin, lang, tba),
                            row.location_name || "",
                          ]
                            .filter(Boolean)
                            .join(" · ")}
              ...
              <div className="-mx-3">
                {filteredPhases.length > 0 && (
                  <CheckboxSelectAllRow
                    ...
                    onChange={(on) =>
                      toggleAll(setSelPhases)(
                        filteredPhases.map((p) => p.id),
                        on
                      )
                    }
          ...
          {rows.length === 0 ? (
            ...
            <ul>
              {rows.map((row) => {
                ...
                const titleValue =
                  type === "concert" && (row.title ?? "").trim().length > 0 ? row.title!.trim() : undefined;
        ...
        <div className="-mx-3">
          ...
          {rows.length === 0 ? (
            <p className="text-sm text-base-content/50 py-2 px-3">
              {allRows.length === 0 ? noVotesLabel : noMatchesLabel}
            </p>
          ) : (
            <ul>
              {rows.map((row) => (
