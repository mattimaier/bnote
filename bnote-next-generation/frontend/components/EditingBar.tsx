...
export function EditingBar({ isNew, saving, onCancel, submitFormId = "entity-edit-form", onSave }: EditingBarProps) {
  const { t } = useI18n();
  const saveLabel = isNew
    ? t("js.common.create") !== "js.common.create"
      ? t("js.common.create")
      : "Create"
    : t("js.common.save") !== "js.common.save"
      ? t("js.common.save")
      : "Save";
    ...
    <div className="z-30 w-full rounded-b-box border border-base-300 border-t-0 bg-[color-mix(in_oklch,var(--primary)_10%,var(--card))] px-4 py-3 shadow-sm text-base-content">
      <div className="mx-auto flex max-w-[1200px] flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {submitFormId ? (
            <button type="submit" form={submitFormId} disabled={saving} className={actionButtonClassNames("primary")}>
          ...
          <ActionButton variant="outline-error" onClick={onCancel} disabled={saving}>
            <X className="h-4 w-4" />
