...
export function ContactDetail() {
  ...
  const groupLabels = useMemo(() => {
    ...
    const labels = contact.groups.map((gid) => groupMap.get(gid) ?? "").filter(Boolean);
  ...
  const titleWithAvatar = (
    <div className="flex items-center gap-3">
      <Avatar email={contact.email} name={title} size={40} variant="solid" className="shrink-0" />
      <span className="truncate">{title}</span>
    </div>
  );
  const birthdayValue =
    contact.birthday && contact.birthday !== "0000-00-00" ? formatDateShortDisplay(contact.birthday, lang) : emptyText;

  const label = (key: string, fallback: string) => (t(key) !== key ? t(key) : fallback);
  const yesLabel = t("js.common.yes") !== "js.common.yes" ? t("js.common.yes") : "Yes";
  const noLabel = t("js.common.no") !== "js.common.no" ? t("js.common.no") : "No";
  const getBadgeClass = (value?: boolean) => (value ? "badge badge-success badge-sm" : "badge badge-error badge-sm");
        ...
        canManageContacts ? (
          <div className="flex items-center gap-2">
            <ActionButton
              variant="outline"
              onClick={() =>
                router.push(`/contacts/integration/?mode=remove&contact=${encodeURIComponent(String(contact.id))}`)
              }
            >
              <Trash2 className="h-4 w-4" />
              {removeFromFutureLabel}
            </ActionButton>
            <DetailEditButton onClick={() => router.push(getEntityPath("contact", contact.id, "edit"))} />
          </div>
        ) : null
      }
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <span className="text-xs font-medium text-base-content/60">{label("js.contacts.lastName", "Last name")}</span>
          <p className="text-sm mt-1">{contact.surname || emptyText}</p>
        </div>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.nickname", "Nickname")}</span>
      ...
      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.birthday", "Birthday")}</span>
        <p className="text-sm mt-1">{birthdayValue}</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <span className="text-xs font-medium text-base-content/60">{label("js.contacts.phone", "Phone")}</span>
          <p className="text-sm mt-1">{contact.phone || emptyText}</p>
        </div>
        <div>
          <span className="text-xs font-medium text-base-content/60">{label("js.contacts.mobile", "Mobile")}</span>
          <p className="text-sm mt-1">{contact.mobile || emptyText}</p>
        </div>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.street", "Street")}</span>
        <p className="text-sm mt-1">{contact.street || emptyText}</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <span className="text-xs font-medium text-base-content/60">{label("js.contacts.zip", "ZIP")}</span>
          <p className="text-sm mt-1">{contact.zip || emptyText}</p>
        </div>
        <div>
          <span className="text-xs font-medium text-base-content/60">{label("js.contacts.city", "City")}</span>
          <p className="text-sm mt-1">{contact.city || emptyText}</p>
        </div>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.company", "Company")}</span>
        <p className="text-sm mt-1">{contact.company || emptyText}</p>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.business", "Business")}</span>
        <p className="text-sm mt-1">{contact.business || emptyText}</p>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.web", "Website")}</span>
        <p className="text-sm mt-1">{contact.web || emptyText}</p>
      </div>

      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.notes", "Notes")}</span>
      ...
      <div>
        <span className="text-xs font-medium text-base-content/60">{label("js.contacts.groups", "Groups")}</span>
      ...
      <div className="rounded-none border-0 p-4 md:rounded-box md:border md:border-base-300 md:p-4 bg-base-100 md:bg-transparent">
        <h3 className="text-sm font-semibold text-base-content">{label("js.profile.privacyTitle", "Visibility")}</h3>
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <div className="mt-1">
              <span className={getBadgeClass(contact.share_email)}>{contact.share_email ? yesLabel : noLabel}</span>
            </div>
          </div>
          <div>
            <span className="text-xs font-medium text-base-content/60">
            ...
            <div className="mt-1">
              <span className={getBadgeClass(contact.share_address)}>{contact.share_address ? yesLabel : noLabel}</span>
            </div>
          </div>
          <div>
            <span className="text-xs font-medium text-base-content/60">
            ...
            <div className="mt-1">
              <span className={getBadgeClass(contact.share_phones)}>{contact.share_phones ? yesLabel : noLabel}</span>
            </div>
          </div>
          <div>
            <span className="text-xs font-medium text-base-content/60">
