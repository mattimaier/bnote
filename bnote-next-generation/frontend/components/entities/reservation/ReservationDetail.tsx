/**
 * BNote Next Generation - Reservation detail view
 *
...
export function ReservationDetail() {
  ...
  const typeConfig = getEventTypeConfig("reservation", t);
  const ReservationIcon = getIcon(typeConfig.icon);
      ...
      <DetailPageHeader
        ...
        subtitle={
          <span className={`inline-flex items-center gap-1.5 event-badge ${typeConfig.badgeClass} w-fit`}>
            <ReservationIcon className="h-3.5 w-3.5 shrink-0" />
            <span>{typeConfig.label}</span>
          </span>
        }
        right={<DetailEditButton onClick={() => router.push(getEntityPath("reservation", item.id, "edit"))} />}
      />
